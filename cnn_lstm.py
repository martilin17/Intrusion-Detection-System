...
ATTACK_MODEL_TAG = "Hierarchical XGBoost + CNN-LSTM"
...
def train_attack_cnn_lstm(
    train_loader: DataLoader,
    val_loader: DataLoader,
    class_weights: torch.Tensor,
    ckpt_prefix: str = "hier_atk4_cnnlstm",
):
    model = CNNLSTMHybrid(
        in_ch=IN_CH,
        n_classes=4,
    ...
    criterion = nn.CrossEntropyLoss(weight=class_weights.to(device), label_smoothing=S2_LABEL_SMOOTH)
    optimizer = torch.optim.Adam(model.parameters(), lr=LR_T)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=EPOCHS_T, eta_min=1e-6)

    best_loss = float("inf")
    patience_count = 0
    stop_epoch = EPOCHS_T
    ckpt_best = CKPT_DIR / f"{ckpt_prefix}_best.ckpt"
    ckpt_last = CKPT_DIR / f"{ckpt_prefix}_last.ckpt"

    print(f"\n{'='*72}\n  ATK4: CNN-LSTM Hybrid | classes=4\n{'='*72}")
    print(
        f"  LSTM hidden={LSTM_HIDDEN} layers={LSTM_LAYERS} "
        f"bidir={LSTM_BIDIR} | LR={LR_T} BS={BATCH_T}"
    )
    ...
    for ep in range(1, EPOCHS_T + 1):
        ...
        if ep <= 10 or ep % 50 == 0 or is_best:
            tag = " *best" if is_best else ""
            print(f"[ATK4 CNNLSTM] Ep {ep:04d} | train {tr_loss:.6f} | val {va_loss:.6f} | patience {patience_count:3d}{tag}")
...
print(f"\n[ATK4 CNN-LSTM] best val loss: {atk_best:.6f}")
